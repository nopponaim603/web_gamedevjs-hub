...
export function createAudio() {
    ...
    function playTone(freq, duration, type, volume, dest, delay = 0, pitchBend = 0) {
        if (!ctx || muted) return;
        ...
        if (pitchBend !== 0) {
            osc.frequency.linearRampToValueAtTime(Math.max(30, freq + pitchBend), startTime + duration);
        }
    ...
    return {
        ...
        frenzyStart() {
            for (let i = 0; i < 4; i++) {
                playTone(440 * (1 + i * 0.25), 0.16, 'sawtooth', 0.05, masterGain, i * 0.07);
            }
