(function(){
  ...
  // 各面"标准顺时针"(从该面外侧看顺时针)对应�� axisOf 旋转的角度符号：
  ...
  // 第一遍：算质心/散布半径/朝外方�� → 造一个透明命中大圆(补空隙) → 画 9 个贴纸点
  FACES.forEach(face=>{
    let sumx=0, sumy=0;
  ...
  // 直接点击某个面的色块簇（贴纸或其间空隙）即可转动该面：以"该面朝��方向"为
  ...
  // 需求：去��� F 面（蓝色）上的 1-9 数字，不再声明序号相关数组、不再生成序号元素。
  ...
  // 在该方向上需要的弧最少，视觉上最接近 90° 的自然弧；同时因为��按 sign 强制选的，
  ...
  /* ============================================================
     9. 键盘快捷键（可切换：���色键 / 标准键 U/D/F/B/L/R）
     ============================================================ */
  const KEY_MAPS = {
    color: {
      'y':['U',false],'Y':['U',true],
